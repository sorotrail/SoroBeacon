...
func TestListChannelsTypeAndEnabledFilters(t *testing.T) {
	...
	all, err := st.ListChannels(ctx, ChannelFilter{})
	require.NoError(t, err)
	require.Len(t, all, 3)

	byType, err := st.ListChannels(ctx, ChannelFilter{Type: "slack"})
	require.NoError(t, err)
	...
	composed, err := st.ListChannels(ctx, ChannelFilter{Type: "slack", EnabledOnly: true})
	require.NoError(t, err)
	require.Len(t, composed, 1)
	assert.Equal(t, slackOn.ID, composed[0].ID)

	unknown, err := st.ListChannels(ctx, ChannelFilter{Type: "not-a-real-type"})
	require.NoError(t, err)
